...
export default function SimpleApp() {
  ...
  const [logs, setLogs] = useState([]); // 3. 변화 감지 -> 리렌더링
  ...
  const increment = useCallback(() => {
    setCount(count + 1);
  }, [count]); // 4. 함수 재정의 => useCallback을 이용해서 재정의 막기
  ...
  const addLog = useCallback((message) => {
    ...
    setLogs((prev) => [...prev, `${timestamp}: ${message}`]); // 2. 로그 배열에 추가
  }, []);

  // useEffect에서 함수들을 의존성으로 사용
  useEffect(() => {
    console.log("increment 함수가 변경됨 - useEffect 실행");
    addLog("increment 함수 변경됨"); // 1. 로그 찍기, 6. ...
    console.log(logs);
  }, [increment, addLog]); // 5. increment 함수 변화되었으니 다시 실행
